...
def main():
	...
	parser.add_argument(
		...
		required=True,
		help=
		"1) Giving Log Links file as input. 2) Giving Log Files file as input.")
	args = parser.parse_args()
	


	with open(args.inputFile, "r") as fileToRead:
	...
	for line in readLines:
		...
		if len(splitStr) == 2:
			...
			partB = re.sub(r"[\n\t\s]", "", splitStr[1])
			partC = re.sub(r"[\n\t\s]", "", splitStr[2])
		testLogLinks.append([partA, partB, partC])

	# Here is where the next stuff should begin
	if args.links:
		logFiles = []
	...
	# Do the rest if we dont have a true value for the links argument	
