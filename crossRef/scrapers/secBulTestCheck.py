...
def getInfoFromUpdatePage(page_url):
	...
	return [file_name_list, testInfoToPrint, bugToPrint, cveToPrint]
	...
	for i in range(1, len(table_rows)):
		...
		if columns[1].a != None:
			if columns[1].a.get("href") != "#asterisk":
				...
				# Append overall list
				if fileRefs != None:
					allFileRefs.append([curCVE, fileRefs[0], fileRefs[1], fileRefs[2], fileRefs[3]])
...
def main():
		...
		for j in range(11, -1, -1):
			...
			if returnFiles != None:
				for i in range(len(returnFiles)):
					...
					securityBulletinFiles.append([first, returnFiles[i][1], returnFiles[i][2], returnFiles[i][3], returnFiles[i][4]])
	...
	cveFilesSorted = [item for items, c in Counter(cveFiles).most_common() for item in [items] * c]
	cveFilesSorted = [d for d in cveFilesSorted if cveFilesSorted.count(d) > 1]

	print(cveFilesSorted)

	prevFile = cveFilesSorted[0]
	uniqueFilesSorted = []
	for i in range(1, len(cveFilesSorted)):
		currFile = cveFilesSorted[i]
		if currFile == prevFile:
			continue
		else:
			uniqueFilesSorted.append(prevFile)
			prevFile = currFile

	print("\n\n")
	print(uniqueFilesSorted)

	testInfoSorted = []
	for cveFile in uniqueFilesSorted:
		for updateInfo in securityBulletinFiles:
			if cveFile in updateInfo[1]:
				if updateInfo[2] == "No Test Info in Desc.":
					testInfoSorted.append([updateInfo[0], cveFile, "No Test Info in Desc."])
				else:
					for testInfo in updateInfo[2]:
						testInfoSorted.append([updateInfo[0], cveFile, testInfo])

	fileToWrite = open("secBulletinFreqTests.txt", "w")
	print("\n\n")
	outputStr = "{0:<25}{1:<40}{2:<50}".format("CVE Reference", "File Name", "Associated Test Information")
	print(outputStr)
	fileToWrite.write(outputStr + "\n")
	for testInfo in testInfoSorted:
		outputStr = "{0:<25}{1:<40}{2:<50}".format(testInfo[0], testInfo[1], testInfo[2])
		print(outputStr)
		fileToWrite.write(outputStr + "\n")

	fileToWrite.close()
			
	#  Can also print Bug ID information and additional CVEs referenced in the description if necessary
	
#	fileToWrite = open("secBulletinTestInfo.txt", "w")
#	print("\n\n")
#	for cveInfo in securityBulletinFiles:
#		outputStr = "{0:<25}{1:<50}".format("CVE Reference", "Updated Files")
#		print(outputStr)
#		fileToWrite.write(outputStr + "\n")
#		for i in range(len(cveInfo[1])):
#			outputStr = "{0:<25}{1:<50}".format(cveInfo[0], cveInfo[1][i])
#			print(outputStr)
#			fileToWrite.write(outputStr + "\n")
#		outputStr = "{0:<25}{1:<50}".format("CVE Reference", "Testing Information")
#		print("\n" + outputStr)
#		fileToWrite.write("\n" + outputStr + "\n")
#		if cveInfo[2] == "No Test Info in Desc.":
#			outputStr = "{0:<25}{1:<50}".format(cveInfo[0], cveInfo[2])
#			print(outputStr)
#			fileToWrite.write(outputStr + "\n")
#		else:
#			for i in range(len(cveInfo[2])):
#				outputStr = "{0:<25}{1:<50}".format(cveInfo[0], cveInfo[2][i])
#				print(outputStr)
#				fileToWrite.write(outputStr + "\n")
#		print("\n")
#		fileToWrite.write("\n\n")
